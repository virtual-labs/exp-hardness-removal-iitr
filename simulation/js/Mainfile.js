...
function addLiquidDrop() {
  ...
  if (liquidLevel >= 1 && bureteTouched == true) {
    ...
    //changing colour of liquid on reaching the desired volume and runonce so that it needs to run only once        
...
function nextpressed() {
  ...
  // Store first titration data locally (no PHP backend)
  const stored = DataStorage.storeTitrationData(
    normality_titrate,
    volume_titrate,
    vadded,
    normality_titrant,
  ...
  if (stored) {
    console.log('First titration data stored locally in localStorage');
    // Redirect to second titration page (part 2)
    window.location.href = './Mytitration2.html';
  } else {
    console.error('Failed to store first titration data');
    alert('Error saving experiment data. Please try again.');
  }
